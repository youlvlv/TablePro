...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_81(st, Dm_build_378)
	if err != nil {
		return err
	}
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_378)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_379)
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	if retSqlType == Dm_build_393 || retSqlType == Dm_build_399 {
	...
	if stmt.paramCount > 0 && args != nil && len(args) > 0 {
		...
		if len(args) == 1 || stmt.dmConn.dmConnector.batchType == 2 ||
			(stmt.dmConn.dmConnector.batchNotOnCall && retSqlType == Dm_build_394) {
			...
			if err != nil {
				if dmErr, ok := err.(*DmError); ok {
					if dmErr.ErrCode == EC_SRC_MULTI_ROWS.ErrCode && !stmt.multiRowsError && retSqlType != Dm_build_395 {
